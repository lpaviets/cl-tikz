...
(defgeneric draw-substitution (base n &key initial))
(defmethod draw-substitution :before (base n &key initial)
  (declare (ignore initial))
  (when (< n 1)
    (error "Number ~S of steps cannot be negative" n)))

(defun draw-substitution-in-file (base name steps &key initial)
  (let ((filename (merge-pathnames (format nil "examples/~A.tex" name))))
    ...
    (with-preamble-to-file (filename) ()
      (with-env (:tikzpicture)
        (draw-substitution base steps :initial initial)))))
